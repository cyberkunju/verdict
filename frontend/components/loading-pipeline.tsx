...
const STAGES = [
  "Queueing analyzer job...",
  "Downloading / trimming source video...",
  "Extracting transcript, face, voice, and rPPG signals...",
  "Scoring multimodal features + text prior...",
  "Generating analyst synthesis...",
...
export function LoadingPipeline({ phase, onComplete }: { phase?: string; onComplete: () => void }) {
  ...
  return (
    <div className="mx-auto max-w-2xl py-12">
      <div className="mb-10 text-center">
        <h2 className="mb-2 font-serif text-3xl text-slate-900">Running VERDICT pipeline</h2>
        <p className="text-slate-500">Transcribing, selecting statement, inferring text prior, and fusing multimodal signals.</p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        <div className="space-y-6">
          {STAGES.map((stage, idx) => {
            ...
            return (
              <motion.div key={stage} initial={{ opacity: 0, x: -10 }} animate={{ opacity: isPending ? 0.4 : 1, x: 0 }} className="flex items-center gap-4">
                <div className="flex-shrink-0">
                  {isCompleted && <CheckCircle2 className="h-6 w-6 text-emerald-500" />}
                  {isCurrent && <Loader2 className="h-6 w-6 animate-spin text-blue-500" />}
                ...
                <div className="flex-1">
                  ...
                  {isCurrent && (
                    <motion.div layoutId="active-bar" className="mt-2 h-1 w-full overflow-hidden rounded-full bg-slate-100">
                      <motion.div
                        initial={{ x: "-100%" }}
                        animate={{ x: "0%" }}
